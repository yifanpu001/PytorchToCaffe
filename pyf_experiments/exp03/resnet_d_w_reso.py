...
if __name__ == '__main__':
    ...
    # """ ResNet_Bottleneck """
    # model = ResNet_Bottleneck(Bottleneck, [3, 4, 6, 3], width=8)
    # model.eval()
    # input_tensor = torch.randn(4, 3, 224, 224)
    # output_tensor = model(input_tensor)
    # print(output_tensor.shape)
    ...
    name = f'exp03/resnet_{args.block}_depth[{str(args.depth_config[0])},{str(args.depth_config[1])},{str(args.depth_config[2])},{str(args.depth_config[3])}]_width{str(args.width)}_re{str(input_size[0])}x{str(input_size[1])}'
    print(f'{save_path}/{name}')
    os.system(f'mkdir {save_path}/{name}')
    with open(f'{save_path}/{name}/model_config.txt', 'w') as fd:
            fd.write(model.__repr__() + '\n')

    caffe_model_name = f'exp03_resnet_{args.block}_depth[{str(args.depth_config[0])},{str(args.depth_config[1])},{str(args.depth_config[2])},{str(args.depth_config[3])}]_width{args.width}_re{input_size[0]}x{input_size[1]}'
